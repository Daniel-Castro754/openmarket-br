...
def test_resolve_ticker_links_b3_instrument_to_cvm_company() -> None:
    service = InstrumentResolutionService(
        instrument_provider=FakeInstrumentProvider(),
        company_provider=FakeCompanyProvider(),
    )

    instrument, company = asyncio.run(service.resolve("PETR4"))
...
def test_resolve_unknown_ticker_fails_without_guessing() -> None:
    service = InstrumentResolutionService(
        instrument_provider=FakeInstrumentProvider(),
        company_provider=FakeCompanyProvider(),
    )
    ...
    try:
        asyncio.run(service.resolve("XXXX3"))
