...
test.describe("OpenMarket BR · screener data-backed behavior", () => {
  test("filters, sorts and opens current-ratio Data Passport", async ({ page }, testInfo) => {
    ...
    await expect(passportLink).toHaveAttribute(
      "href",
      new RegExp(`/ativos/${ticker}/indicadores\\?passport=current-ratio#data-passport$`),
    );
    ...
    const sortedCurrentRatioColumn = sortedHeaders.findIndex((label) =>
      label.includes("Liquidez Corrente"),
    );
    expect(sortedCurrentRatioColumn).toBeGreaterThanOrEqual(0);

    const sortedPassportLink = sortedRow
      .locator("td")
      .nth(sortedCurrentRatioColumn)
      .getByRole("link");
    ...
    await expect(
      page.getByRole("heading", { name: `Liquidez Corrente · ${ticker}` }),
    ).toBeVisible();
    await expect(page.getByRole("heading", { name: "Dados usados no cálculo" })).toBeVisible();
    await expect(
      page.getByText("current_assets / current_liabilities", { exact: true }),
    ).toBeVisible();
